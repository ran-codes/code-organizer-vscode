...
- [ ] **#50** reveal never fires on a refreshing sync pass
  ...
  - [x] Dev — on branch `issue-50`. 109 tests passing, `npm run compile` clean.
...
- [ ] **#51** sections under a collapsed parent never cached
  ...
  - [x] Dev — no extra code, as predicted; create-on-miss covers it and a test pins
        the collapsed-parent chain (child instance + `getParent`). **Still needs the
        `master` repro** before it closes in the PR rather than as not-a-bug.
...
so what is left is two PRs: #50+#51 and #54.
...
| 6 | [#51](https://github.com/ran-codes/code-organizer-vscode/issues/51) collapsed parents | Bug | *in* `TODO__issue_50.md` | **`master` repro** — then close in #50's PR | **#50** | Same root cause, other side of it: VS Code never calls `getChildren()` on a collapsed parent, so descendants are never cached. Create-on-miss covers it with no extra code. Never reproduced — repro on `master` first, then close in #50's PR, or close as not-a-bug. Own doc only if it survives the fix. |
