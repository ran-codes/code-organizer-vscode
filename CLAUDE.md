...
│   ├── test/                    # Mocha suites — one per comment syntax, plus per-util
...
| `src/test/` | Mocha suites — one per comment syntax (hash, double-slash, JSX, md, quarto, sql) plus one per non-parser util (`sectionTree.test.ts`). Compiled to `out/test/` before running. | ❌ |
